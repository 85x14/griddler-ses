# based on https://github.com/thoughtbot/griddler-sendgrid/blob/master/spec/griddler/sendgrid/adapter_spec.rb, MIT License
require 'spec_helper'
...
describe Griddler::Ses::Adapter, '.normalize_params' do
  before do
    allow(Griddler::Ses::Adapter).to receive(:raw_request).and_return(request)
  end

  it_should_behave_like 'Griddler adapter', :ses, {}
  ...
  let(:sns_message) {
    {
      ...
      "mail": {
        ...
        "headers": [
          {
            "name": "Received",
            "value": "from [192.168.10.141] ([100.36.63.67]) by smtp.gmail.com with ESMTPSA id p189sm25370923pfb.51.2016.04.12.15.02.32 for <reply@replies.staging.coupacontracts.com> (version=TLSv1/SSLv3 cipher=OTHER); Tue, 12 Apr 2016 15:02:32 -0700 (PDT)"
          },
          {
            "name": "To",
